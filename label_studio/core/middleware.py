    ...
    def process_request(self, request) -> None:
        ...
        if active_org:
            ...
            if (current_time - last_login) > org_max_session_age:
                logger.info(
                    f"1 --Request is too far from last login {current_time - last_login:.0f} > {settings.MAX_SESSION_AGE}; logout"
                )
                # logout(request)
            ...
            if (current_time - last_login) > settings.MAX_SESSION_AGE:
                logger.info(
                    f"2-- Request is too far from last login {current_time - last_login:.0f} > {settings.MAX_SESSION_AGE}; logout"
                )
                # logout(request)
